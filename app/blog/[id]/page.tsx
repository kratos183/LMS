...
import { BlogPost, BlogContentBlock, BlogComment } from '@/types';
import { 
...
export default function BlogSinglePage() {
  ...
  const numericId = Number(blogId) || 1;
  ...
  useEffect(() => {
    ...
    const fetchRecentBlogs = async () => {
      ...
      setRecentBlogs((data as BlogPost[]) || []);
    };
  ...
  const content: any[] = Array.isArray(blog.content) ? blog.content : (typeof blog.content === 'string' ? [blog.content] : []);
  const tags: string[] = Array.isArray(blog.tags) ? blog.tags : [];
  const comments: BlogComment[] = Array.isArray(blog.comments) ? blog.comments : [];
  ...
  const renderContent = () => {
    ...
    if (typeof firstItem === 'object' && firstItem !== null) {
      return content.map((block: BlogContentBlock, index: number) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
              <NextLink href={`/blog/${Math.max(1, numericId - 1)}`} className="flex items-center gap-4 p-6 border border-gray-200 rounded-lg hover:border-orange-500 hover:shadow-md transition group">
              ...
              <NextLink href={`/blog/${numericId + 1}`} className="flex items-center gap-4 p-6 border border-gray-200 rounded-lg hover:border-orange-500 hover:shadow-md transition group flex-row-reverse text-right">
                <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-orange-500 group-hover:text-white transition">
